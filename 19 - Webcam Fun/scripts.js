...
function getVideo() {
    navigator.mediaDevices.getUserMedia({ video: true, audio: false })
        ...
        .catch(err => {
            console.error(`Web Cam access denied, ${err}`)
        });
}
...
function rgbSplitEffect(pixels) {
    for (let i = 0; i < pixels.data.length; i += 4) {
        ...
        pixels.data[i + 100] = pixels.data[i + 1];  // gree
...
function takePhoto() {
    ...
    link.href = data;
    link.setAttribute('download', 'CamImage');
    link.innerHTML = `<img src="${data}" alt="CamImage" />`
